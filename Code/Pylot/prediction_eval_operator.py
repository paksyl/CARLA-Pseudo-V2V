...
# Global lists to allow accessing ground and predicted trajectory data before conversion to world coordinates
#GROUND_TRAJECTORY_BEFORE_CONVERSION = []
#PRED_TRAJECTORY_BEFORE_CONVERSION = []
...
class PredictionEvalOperator(erdos.Operator):
    ...
    def on_watermark(self, timestamp: Timestamp):
        ...
        if len(self._predictions) == self._flags.prediction_num_future_steps:
            ...
            GROUND_TRAJECTORY_BEFORE_CONVERSION_LIST = []
            for obstacle_trajectory in tracking_msg.obstacle_trajectories:
                GROUND_TRAJECTORY_BEFORE_CONVERSION_LIST.append(obstacle_trajectory) # Append ground data to global list
                #GROUND_TRAJECTORY_BEFORE_CONVERSION_LIST[obstacle_trajectory.id] = obstacle_trajectory
            ...
            self._calculate_metrics(timestamp, ground_trajectories_dict,
                                    self._predictions[0].predictions, GROUND_TRAJECTORY_BEFORE_CONVERSION_LIST, self.PRED_TRAJECTORY_BEFORE_CONVERSION_LIST)

        # Convert the prediction to world coordinates and append it to the
        # queue.
        self.PRED_TRAJECTORY_BEFORE_CONVERSION_LIST = []
        for obstacle_prediction in prediction_msg.predictions:
            self.PRED_TRAJECTORY_BEFORE_CONVERSION_LIST.append(obstacle_prediction) # Append prediction to global list
    ...
    def _calculate_metrics(self, timestamp: Timestamp, ground_trajectories,
                           predictions, GROUND_TRAJECTORY_BEFORE_CONVERSION, PRED_TRAJECTORY_BEFORE_CONVERSION):
        ...
        if vehicle_cnt > 0:
            ...
            # Log ground trajectory and predicted trajectory coordinates for debugging
            #Before conversion to world coordinates
            #for i, ground_coord_before in enumerate(GROUND_TRAJECTORY_BEFORE_CONVERSION.trajectory):
            #    self._logger.info('Ground Coordinate {} before conversion is: {}'.format(i, str(ground_coord_before)))
            #    
            #for j, pred_coord_before in enumerate(PRED_TRAJECTORY_BEFORE_CONVERSION.predicted_trajectory):
            #    self._logger.info('Predicted Coordinate {} before conversion is: {}'.format(j, str(pred_coord_before)))            
                     
            # After conversion to world coordinates
            for i, ground_coord in enumerate(ground_trajectory):
                self._logger.info('Ground Coordinate {} is: {}'.format(i, str(ground_coord)))
                
            for j, pred_coord in enumerate(predicted_trajectory):
                self._logger.info('Predicted Coordinate {} is: {}'.format(j, str(pred_coord)))                
