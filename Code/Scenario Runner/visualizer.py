...
from srunner.scenariomanager.timer import GameTime # To get current time
...
class Visualizer(object):
    ...
    def render(self):
        ...
        if self._cv_image_actor is not None and self._cv_image_bird is not None:
            ...
            self.redis.set('SR_Time', str(current_time))
            current_time = str(int(current_time))+" sec" # Current time
            current_time = ' '*(7-len(current_time)) + current_time #
            im_v = cv2.putText(im_v, text, (830, 310), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2, cv2.LINE_AA)
            cv2.putText(im_v, current_time, (830, 350), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2, cv2.LINE_AA) #
            cv2.imshow("", im_v)
            cv2.waitKey(1)
